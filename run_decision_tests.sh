...
if [ $# -eq 0 ]
then
    echo "usage: ./run_tests.sh solver-cmd inputs ref-solutions timeout memout decision-problem-task[s]"
    exit 0
fi
...
do
    ...
    do
        ...
        ARGUMENT=$(cat "$REFS"/"$PROBLEM_FILE_NAME".apx.arg)

        runsolver -w $BURNER/watcher.txt -v $BURNER/varfile.txt -o $BURNER/solver.o -W $TIMEOUT --vsize-limit $MEMOUT $SOLVER -fo tgf -f $PROBLEM_FILE -p $PROBLEM_TASK -a $ARGUMENT
    ...
    if cmp -s "$BURNER"/solver.o "$REFS"/"$PROBLEM_FILE_NAME".apx-"$PROBLEM_TASK".out
    then
        MATCH="Solutions Match."
    else
        MATCH="Solutions DO NOT match."
    fi
